        ...
        <li v-for="item in items" :key="`${item.entry.type}-${item.entry.id}`" class="flex items-center gap-4 p-4">
          <NuxtLink :to="item.link ? localePath(item.link) : ''" class="shrink-0" :class="item.link ? '' : 'pointer-events-none'">
            <NuxtImg
              ...
              loading="lazy"
            />
            <div v-else class="w-24 h-16 flex items-center justify-center bg-ink-900" aria-hidden="true">
              <svg class="w-5 h-5 text-bone-400/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
